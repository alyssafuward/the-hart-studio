...
const events = [
  ...
  { name: "Live Build Launch Session", date: new Date("2026-06-19T15:30:00Z"), type: "Live builds", sub: "Free", description: "A free 90-minute live session. 1 hour live build demo in Claude and Claude Code, open to anyone." },
  { name: "Tech Setup Session", date: new Date("2026-06-19T17:00:00Z"), type: "Live builds", sub: "Paid", description: "30-min hands-on group tech setup for paid subscribers. Get your Claude Code + GitHub stack ready." },
  ...
  { name: "Office Hours – Wed", date: new Date("2026-07-01T15:00:00Z"), type: "Office Hours", sub: "Paid", description: "Open office hours. Sign up for a 30-min slot focused on tech setup and wherever you're getting blocked." },
  { name: "Office Hours – Thu", date: new Date("2026-07-02T15:00:00Z"), type: "Office Hours", sub: "Paid", description: "Open office hours. Sign up for a 30-min slot focused on tech setup and wherever you're getting blocked." },
  { name: "Live Build #1 – Getting started", date: new Date("2026-07-17T15:30:00Z"), type: "Live builds", sub: "Paid", description: "End-to-end build process on Open Room. From idea to GitHub to live." },
  { name: "Live Build #2 – Create a habit", date: new Date("2026-07-31T15:30:00Z"), type: "Live builds", sub: "Paid", description: "Build on what we made in Session 1. Add to it, elaborate, iterate." },
  { name: "Live Build #3 – Make it your own", date: new Date("2026-08-07T15:30:00Z"), type: "Live builds", sub: "Paid", description: "Set up your own repo and your own project." },
  { name: "Office Hours – Aug", date: new Date("2026-08-07T15:00:00Z"), type: "Office Hours", sub: "Paid", description: "Open office hours." },
  { name: "Live Build #4 – Onboarding", date: new Date("2026-08-14T15:30:00Z"), type: "Live builds", sub: "Paid", description: "Build an onboarding flow." },
  { name: "Live Build #5 – Focused build", date: new Date("2026-08-21T15:30:00Z"), type: "Live builds", sub: "Paid", description: "A structured walkthrough build — Alyssa-led from start to finish." },
  { name: "Live Build #6 – Build and share", date: new Date("2026-08-28T15:30:00Z"), type: "Live builds", sub: "Paid", description: "Everyone builds from the same prompt. Then we share what we made." },
  { name: "Office Hours – Sep", date: new Date("2026-09-04T15:00:00Z"), type: "Office Hours", sub: "Paid", description: "Open office hours." },
  { name: "Live Build #7 – Onboarding", date: new Date("2026-09-11T15:30:00Z"), type: "Live builds", sub: "Paid", description: "New member onboarding session." },
  { name: "Office Hours – Oct", date: new Date("2026-10-16T15:00:00Z"), type: "Office Hours", sub: "Paid", description: "Open office hours." },
];

const PAGE_SIZE = 6;
const upcoming = events.filter((e) => e.date >= today).sort((a, b) => a.date - b.date);
const allMonths = [...new Set(upcoming.map((e) => `${e.date.getFullYear()}-${e.date.getMonth()}`))]
  .map((key) => {
    const e = upcoming.find((ev) => `${ev.date.getFullYear()}-${ev.date.getMonth()}` === key);
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}
          style={{ padding: "7px 16px", borderRadius: 8, border: "1.5px solid #E5E5E5", background: "#fff", color: page === 0 ? "#CCC" : "#555", fontSize: 12, cursor: page === 0 ? "default" : "pointer", fontFamily: "inherit" }}>← Prev</button>
        <span style={{ fontSize: 11, color: "#BBB", fontFamily: "monospace" }}>{page + 1} / {totalPages}</span>
        <button onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1}
          style={{ padding: "7px 16px", borderRadius: 8, border: "1.5px solid #E5E5E5", background: "#fff", color: page === totalPages - 1 ? "#CCC" : "#555", fontSize: 12, cursor: page === totalPages - 1 ? "default" : "pointer", fontFamily: "inherit" }}>Next →</button>
      </div>
    </div>
  );
}
...
function CalendarView({ onEventClick }) {
  ...
  const currentMonth = allMonths[monthIndex];
  const year = currentMonth.getFullYear();
  const month = currentMonth.getMonth();
  const daysInMonth = getDaysInMonth(year, month);
  const firstDay = getFirstDay(year, month);
  const monthEvents = upcoming.filter((e) => e.date.getFullYear() === year && e.date.getMonth() === month);
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <button onClick={() => setMonthIndex((i) => Math.max(0, i - 1))} disabled={monthIndex === 0}
          style={{ padding: "6px 12px", borderRadius: 8, border: "1.5px solid #E5E5E5", background: "#fff", color: monthIndex === 0 ? "#CCC" : "#555", fontSize: 12, cursor: monthIndex === 0 ? "default" : "pointer", fontFamily: "inherit" }}>←</button>
        <span style={{ fontSize: 15, fontWeight: 600, color: "#1A1A1A" }}>{currentMonth.toLocaleDateString("en-US", { month: "long", year: "numeric" })}</span>
        <button onClick={() => setMonthIndex((i) => Math.min(allMonths.length - 1, i + 1))} disabled={monthIndex === allMonths.length - 1}
          style={{ padding: "6px 12px", borderRadius: 8, border: "1.5px solid #E5E5E5", background: "#fff", color: monthIndex === allMonths.length - 1 ? "#CCC" : "#555", fontSize: 12, cursor: monthIndex === allMonths.length - 1 ? "default" : "pointer", fontFamily: "inherit" }}>→</button>
      </div>
              ...
              {dayEvents.map((ev, j) => {
                ...
                return (
                  <div key={j} onClick={() => onEventClick(ev)}
                    ...
                    {ev.name.replace("Live Build Launch Session", "Launch").replace("Tech Setup Session", "Tech Setup").replace("Office Hours", "OH").replace(/Live Build #\d+ – /, "")}
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div style={{ fontFamily: "Georgia, serif", background: "#FAFAF8", minHeight: "100vh", padding: "32px 24px", maxWidth: 680, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <p style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#999", margin: "0 0 6px" }}>The HART Studio</p>
      ...
      {next && (
        <div onClick={() => setSelectedEvent(next)}
      ...
      {view === "list"
        ? <ListView onEventClick={setSelectedEvent} />
